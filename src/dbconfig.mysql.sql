...
create table message(rowid bigint not null auto_increment, uname varchar(30), frm varchar(30), ctime DATETIME not null DEFAULT CURRENT_TIMESTAMP, msg text, foreign key (uname) references user(uname), foreign key(frm) references user(uname), primary key(rowid));
